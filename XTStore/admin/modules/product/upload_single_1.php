...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (!in_array(strtolower($type_file), $type_fileAllow)) {
        $error['file_1'] = "File bạn vừa chọn hệ thống không hỗ trợ, bạn vui lòng chọn hình ảnh";
    }
    //Kiểm tra kích thước file
    $size_file = $_FILES['file_1']['size'];
    if ($size_file > 5242880) {
        $error['file_1'] = "File bạn chọn không được quá 5MB";
    }
// Kiểm tra file đã tồn tại trê hệ thống
    if (file_exists($target_file)) {
        $error['file_1'] = "File bạn chọn đã tồn tại trên hệ thống";
    }
//
    if (empty($error)) {
