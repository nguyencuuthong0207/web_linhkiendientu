...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $number_rows = db_num_rows("SELECT * FROM product where cat_id= $id and status = 1 ORDER BY `product_name` ASC");
//    echo $number_rows;
} else {
...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $list_product = get_product_A_Z_cate($start, $num_per_page, $id);
} else {
    $list_product = get_product_A_Z($start, $num_per_page);
}
...
<!--                                <button type="submit">Lọc</button>-->
            ...
            <div class="section" id="paging-wp">
                <div class="section-detail clearfix">
                    <?php
                    $id = $item['cat_id'];
                    echo get_pagging_category_product($num_page, $page, "?mod=sort&act=A-Z", $id);
                    ?>
                </div>
            </div>
        </div>
